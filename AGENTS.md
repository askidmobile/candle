...
- **MTP на Qwen3.8 подключён и бит-эксактен (2026-08-19), но БЕЗ ускорения:** контракт параметрический (`MtpProfile` из метаданных, префикс blk.{block_count-nextn}), `load_mtp(тот же GGUF)`. Гейт: `run_mtp_gate.bat <gguf> <gguf> 1` -> pass. A/B (`run_mtp_probe.bat <gguf> <gguf|-> ids 128 1`): 16.3 vs 18.8 tok/s, принимаемость 84/176. Причина замедления — verify в scheduler.rs::speculative_slot ПОСЛЕДОВАТЕЛЬНЫЙ (один target-шаг на каждый драфт-токен, работа таргета не амортизируется). Для ускорения нужен batched verify: один target-forward на K+1 позиций + rollback DeltaNet-state через checkpoint_slot_batched + re-run принятого префикса одним чанком. На 27B гейт/пробу гонять только с 1 слотом.
