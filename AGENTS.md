...
- **MTP batched verify на Qwen3.8 — 1.42x (2026-08-19): 26.5 vs 18.6 tok/s, bit-exact.** Verify = один multi-token forward через batched decode-путь (slots=[slot;K], positions=pos..pos+K): attention batched decode уже корректен для K строк одного слота (append по cache_len), DeltaNet сериализуют `_seq`-ядра (delta_rule_batched.cu: внутренний цикл по строкам, state по-канально/по-столбцово — те же инструкции, бит-в-бит). При m<K — restore checkpoint + re-run префикса одним чанком (без D2H). **K (QWEN36_MTP_WIDTH, default 3) держать ЧУТЬ НИЖЕ типичной длины всплеска принимаемости: при m==K re-run пропускается.** K=4 на 27B МЕДЛЕННЕЕ baseline (15.1 tok/s — re-run горит в ~90% раундов), K=3 — 26.6. Бюджет раунда мерить QWEN36_MTP_TIMING=1 (begin/draft/verify/sample/accept/commit); маржинальная цена verify-строки ~18ms (стрим SSM-state + per-row attention host) — батч амортизирует только ~35ms/шаг фиксированных затрат. Rust-цикл по строкам вместо seq-ядер = -1.6K host-ops = хуже последовательного verify (14.2 tok/s, не повторять). На 27B гейт/пробу гонять только с 1 слотом.
