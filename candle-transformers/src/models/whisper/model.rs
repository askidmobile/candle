        ...
        let (k, v) = match xa {
            ...
            Some(x) => {
                ...
                if let Some((k, v)) = &self.kv_cache {
                    ...
                    // Экономия ~240 МБ для Whisper large-v3 (32 decoder layers).
                    let working_dtype = q.dtype();
                    (k.to_dtype(working_dtype)?, v.to_dtype(working_dtype)?)
                } else {
                    let k = self.key.forward(x)?;
                    let v = self.value.forward(x)?;
                    // Хранение KV-cache в f16 для экономии памяти.
                    // Cross-attention cache статический (записывается 1 раз, читается N раз).
