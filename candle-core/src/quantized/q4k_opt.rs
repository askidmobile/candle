...
// ════════════════════════════════════════════════════════════════════════════════
// GPU upload + dispatch bridge (только под metal feature)
// ════════════════════════════════════════════════════════════════════════════════
...
mod metal_impl {
    ...
    /// T-275 Phase 4: dispatch optimized Q4_K_M matmul напрямую (минуя CustomOp1 path).
    ///
    /// Эквивалент `qmatmul.forward(xs)` но через `kernel_mul_mm_q4_K_f32_opt` с
    /// pre-packed `scales_repacked` buffer.
    ///
    /// Требования (вызывающий должен предварительно проверить):
    /// - `qtensor.dtype() == GgmlDType::Q4K`
    /// - `qtensor.storage` — Metal (модель загружена на Metal device)
    /// - `xs.dtype() == DType::F32`
    /// - `xs` на Metal device
    /// - Dimensions aligned: M%64==0 и N%32==0 (FAST_PATH alignment)
    ///
    /// Возвращает `Tensor` с Metal F32 storage эквивалентного shape что
    /// производит `QMatMul::forward(xs)`.
    pub fn matmul_q4k_opt_metal(
        ...
        // dst shape computation эквивалентна тому что делает QStorage::fwd.
        ...
        // 4D-aligned layout для weight (как в существующем fwd).
