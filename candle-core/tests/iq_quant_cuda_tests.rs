...
fn cuda_fa2_paged_qwen35_dims() -> Result<()> {
    ...
    let num_blocks = 4usize; // enough pages for len

    let q = Tensor::randn(0f32, 1.0, (b, n_head, hd), &device)?.to_dtype(DType::F16)?;
    // Sequential K/V [len, n_kv, hd] → paged pool [num_blocks, page, n_kv, hd]
    ...
    let block_table = Tensor::from_vec((0..num_blocks as u32).collect::<Vec<_>>(), (1, num_blocks), &device)?;

    let scale = 1.0 / (hd as f64).sqrt();
    let out_paged = candle_flash_attn::flash_attn_varlen_paged_windowed(
        &q, &k_pool, &v_pool, &seqlens_q, &seqlens_k, &block_table,
        None, 1, len, scale as f32, None, None, page, None,
    )?;
    // Reference: plain flash_attn [1, n_head, 1, hd] vs [1, len, n_kv, hd]
